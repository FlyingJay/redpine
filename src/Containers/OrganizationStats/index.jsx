...
export class OrganizationStats extends React.PureComponent {
  constructor(props) {
    super(props) 
    this.state = {
      infoTab: 1,
      ...
      <App requireAuth>
        <Modal show onClose={() => this.props.history.goBack()}>
          { this.props.organization 
            ? <MyPageWrapper>
                <Title title={"Statistics for " + organization.title} summary="Insights for your organization...with more coming soon!" />
                <TabOptions>
                ...
                <LoadingIndicator loading={this.props.stats_loading}>
                  <TabFrame show={this.state.infoTab == 1}>
                    <Graph title="Top 10 Collaborators" summary={"Who do you work with most often?"} last>
  ...
  componentDidMount() {
    ...
    this.props.loadStats(organization_id)
  }
...
const mapDispatchToProps = (dispatch) => {
  return {
    loadStats: (organization_id) => {
      dispatch(actions.loadStats(organization_id))
    }
  }
}
